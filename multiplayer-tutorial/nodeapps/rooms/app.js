...
io.on('connection', function(socket){
  ...
  socket.on('new room', function(room){
	  ...
	  socket.room = room; // custom data item
	  socket.join(room); // join is a socket.io method
  ...
  socket.on('chat message', function(data){
    io.in(data.room).emit('chat message', `${data.name}: ${data.msg}` ); // send to the ones in the particular room
  });
  socket.on('set username', function(name){ 
	  console.log(`username set to ${name}(${socket.id})`);
	  socket.username = name; // custom data item
  });
});
